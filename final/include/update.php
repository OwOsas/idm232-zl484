...
if(isset($_POST['submit'])){
    ...
    if(!recipeNameExists($conn, $title)){
        header('Location: ../create.php?error=recipeNotExists');
    }
    elseif(!($fileError === 0) && !($fileError == 4)){
        echo "An Error Has Occurred";

    }
    elseif(!(in_array($fileExt, $allowed_file_type)) && !($fileError == 4)){
        echo "Wrong Type";
    }
    elseif(!($fileSize < $allowed_file_size) && !($fileError == 4)){
        ...
        updateRecipe($conn, $ID, $title, $fileNewName, $difficulty, $region, $type, $prepTime, $cookTime, $ingredients, $origin, $steps);
    }
}